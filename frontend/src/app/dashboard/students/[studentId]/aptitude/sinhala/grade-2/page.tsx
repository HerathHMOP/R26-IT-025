...
export default function Grade2SinhalaAptitudePage() {
  ...
  useEffect(() => {
    ...
    async function loadStudent() {
      try {
        ...
        setSubjectId(dashboard.subjects.find((subject) => subject.code === "sinhala")?.id ?? null);
  ...
  function getActivityMark(activity: (typeof grade2SinhalaActivities)[number]): 0 | 1 {
    ...
    if (activity.type === "word_boxes") {
      ...
      const allRowsCorrect =
        ...
        (activity.wordBoxRows || []).every((row) => selectedMap[row.key] === row.answer);
      return allRowsCorrect ? 1 : 0;
    }
    ...
    if (activity.type === "arrange_words") {
      ...
      const allRowsCorrect =
        ...
        (activity.arrangeWordRows || []).every((row) => {
          const builtWords = (selectedMap[row.key] || "").split(WORD_SEPARATOR).filter(Boolean);
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    try {
      const correctAnswers = grade2SinhalaActivities.reduce((count, activity) => count + getActivityMark(activity), 0);
      const session = await startExamSession(studentId, grade2SinhalaActivities.length);
  ...
  const answeredCount = grade2SinhalaActivities.reduce((count, activity) => {
    ...
    if (activity.type === "word_boxes") {
      const selections = matchAnswers[activity.id] || {};
      return count + (Object.keys(selections).length === (activity.wordBoxRows || []).length ? 1 : 0);
    }
    if (activity.type === "match_letters") {
      const selections = matchAnswers[activity.id] || {};
    ...
    if (activity.type === "arrange_words") {
      const selections = matchAnswers[activity.id] || {};
      const completeRows = (activity.arrangeWordRows || []).filter((row) => {
        const builtWords = (selections[row.key] || "").split(WORD_SEPARATOR).filter(Boolean);
        return builtWords.length === row.words.length;
      }).length;
      return count + ((activity.arrangeWordRows || []).length > 0 && completeRows === (activity.arrangeWordRows || []).length ? 1 : 0);
    }
    return count;
  }, 0);

  function isActivityComplete(activity: (typeof grade2SinhalaActivities)[number]): boolean {
    ...
    if (activity.type === "word_boxes") {
      const selections = matchAnswers[activity.id] || {};
      return Object.keys(selections).length === (activity.wordBoxRows || []).length;
    }
    if (activity.type === "match_letters") {
      const selections = matchAnswers[activity.id] || {};
  ...
  useEffect(() => {
    const current = grade2SinhalaActivities[currentActivityIndex];
    if (!current || !isActivityComplete(current)) return;
    if (currentActivityIndex >= grade2SinhalaActivities.length - 1) return;
    setCurrentActivityIndex((prev) => Math.min(prev + 1, grade2SinhalaActivities.length - 1));
  ...
  const uiLang: AptitudeUiLang = "sinhala";
  const answeredLine = playZoneFinishedLine(answeredCount, grade2SinhalaActivities.length, uiLang);

  return (
    <main className={playZoneMainClassNames(uiLang)}>
      <header className="dashboard-topbar kid-aptitude-topbar">
        <div>
          <p className="dashboard-eyebrow kid-aptitude-eyebrow">{playZoneEyebrow("sinhala", uiLang)}</p>
          <h1 className="title dashboard-title kid-aptitude-title">{playZoneTitle(2, "sinhala", uiLang)}</h1>
          <p className="kid-aptitude-kicker">{playZoneKicker("sinhala", uiLang)}</p>
          <p className="subtitle kid-aptitude-subtitle">
            {playZoneSubtitle(studentName, grade2SinhalaActivities.length, uiLang)}
          </p>
        </div>
          ...
          {!result && grade === 2 ? (
            <article className="dashboard-item subject-item kid-aptitude-card">
              <div className="subject-item-header kid-aptitude-card-header">
                <span className="kid-activity-badge" title="වත්මන් පියවර">
                  <span className="kid-activity-badge-emoji" aria-hidden>
                    ★
                  </span>
                  {playZoneStepBadge(currentActivityIndex, grade2SinhalaActivities.length, uiLang)}
                </span>
              </div>
              ...
              <div className="students-grid section-top">
                {(() => {
                  const activity = grade2SinhalaActivities[currentActivityIndex];
                  ...
                  return (
                    <div key={activity.id}>
                      ...
                      {activity.type === "image_rows" ? (
                        <div className="number-rows-grid section-top">
                          ...
                          {(activity.imageRows || []).map((row) => (
                            <div key={row.key} className="number-row-card">
                              {row.label ? <div className="match-left">{row.label}</div> : null}
                        ...
                        <div className="number-rows-grid section-top">
                          {(activity.wordBoxRows || []).map((row) => (
                            <div key={row.key} className="word-box-row-card">
                              {(() => {
                                const pattern =
                                  row.patternRows ||
                                  [[row.topRow[0], row.topRow[1], row.topRow[2]], [row.sideColumn[0]], [row.sideColumn[1]]];
                                const blankCell = row.blankCell || { row: 0, col: 0 };
                                const colCount = Math.max(...pattern.map((cells) => cells.length));
                                return (
                                  <div className="word-box-shape" style={{ gridTemplateColumns: `repeat(${colCount}, 42px)` }}>
                                    {pattern.flatMap((cells, rowIdx) =>
                                      Array.from({ length: colCount }).map((_, colIdx) => {
                                        const value = cells[colIdx] ?? null;
                                        if (value == null) {
                                          return <div key={`${row.key}-${rowIdx}-${colIdx}`} className="word-box-empty" />;
                                        }
                                        const isBlank = blankCell.row === rowIdx && blankCell.col === colIdx;
                                        return (
                                          <div key={`${row.key}-${rowIdx}-${colIdx}`} className="word-box-cell">
                                            {isBlank ? matchAnswers[activity.id]?.[row.key] || value : value}
                                          </div>
                                        );
                                      })
                                    )}
                              ...
                              <p className="student-meta">{row.optionHint || `(${row.options.join(" / ")})`}</p>
                            </div>
                          ))}
                        </div>
                        ...
                        <div className="match-grid section-top">
                          ...
                          <p className="student-meta">වචනයක් තෝරලා, ගැලපෙන වාක්‍ය කොටසට ටැප් කරන්න.</p>
                          {(activity.leftItems || []).map((left) => (
                            <div key={left} className="match-row">
                              ...
                              >
                                {matchAnswers[activity.id]?.[left] || "ගැලපෙන වචනය තෝරන්න"}
                              </button>
                            </div>
                          ))}
                        </div>
                        ...
                        <div className="number-rows-grid section-top">
                          {(activity.arrangeWordRows || []).map((row, index) => {
                            const builtWords = (matchAnswers[activity.id]?.[row.key] || "").split(WORD_SEPARATOR).filter(Boolean);
                            const builtSentence = builtWords.join(" ");
                            return (
                              <div key={row.key} className="word-box-row-card">
                                <div className="match-left">{index + 1}) {row.words.join(" / ")}</div>
                                <button
                                  type="button"
                                  className={`match-target ${builtWords.length > 0 ? "match-target-filled" : ""}`}
                                  onClick={() =>
                                    setMatchAnswers((prev) => ({
                                      ...prev,
                                      [activity.id]: {
                                ...
                                >
                                  {builtSentence || "වචන ටැප් කර වාක්‍යය සකස් කරන්න"}
                                </button>
                                <div className="choice-pool">
                                  {row.words.map((word) => {
                                    const alreadyPicked = builtWords.includes(word);
                                    return (
                                      <button
                                        key={`${row.key}-${word}`}
                                        type="button"
                                        className={`number-option-btn ${alreadyPicked ? "number-option-btn-selected" : ""}`}
                                        disabled={alreadyPicked}
                                        onClick={() =>
                                          setMatchAnswers((prev) => {
                                            const currentWords = (prev[activity.id]?.[row.key] || "").split(WORD_SEPARATOR).filter(Boolean);
                                            const nextWords = [...currentWords, word];
                                            return {
                                              ...prev,
                                              [activity.id]: {
                                                ...(prev[activity.id] || {}),
                                                [row.key]: nextWords.join(WORD_SEPARATOR)
                                              }
                                            };
                                          })
                                        }
              ...
              {answeredCount === grade2SinhalaActivities.length ? (
